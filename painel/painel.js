(async function(){
  ...
  function renderList(list){
    ...
    if(!list.length){
      prodList.innerHTML='<div class="muted">Nenhum produto</div>';
      return;
    }
    ...
    list.forEach(p=>{
      ...
      row.innerHTML = `
        ...
        <div>
          ...
          <button class="btn" style="background:#ff6b6b;color:#fff" data-id="${p.id}" data-action="del">Excluir</button>
        </div>
      `;
  ...
  function onProdAction(e){
    ...
    if(act==='edit') editProduct(id);
      ...
      if(confirm('Excluir?')) deleteProduct(id);
    }
  }
  ...
  function clearForm(){
    $('prod-id').value='';
    $('prod-nome').value='';
    $('prod-categoria').value='novo';
    $('prod-estoque').value='1';
  ...
  function renderStats(list){
    ...
    statNovos.textContent = list.filter(x=>x.categoria==='novo').length;
    statSemi.textContent = list.filter(x=>x.categoria==='seminovo').length;
    ...
    clicksChart = new Chart(clicksCtx,{
      type:'bar',
      data:{ labels, datasets:[{ label:'Cliques WhatsApp', data, backgroundColor:'rgba(11,110,168,0.8)' }] },
  ...
  navProdutos.addEventListener('click', ()=>{
    ...
    $('page-title').textContent='Produtos';
  });

  navStats.addEventListener('click', ()=>{
    pageProdutos.style.display='none';
    pageStats.style.display='block';
    $('page-title').textContent='Estatísticas';
    if(produtos.length) renderStats(produtos);
  });
